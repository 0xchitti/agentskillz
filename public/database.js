...
export class Database {
    ...
    static addSkill(skill) {
        ...
        const existingSkills = SKILLS_CACHE.filter(s => s.agentId === skill.agentId);
        if (existingSkills.length >= 1) {
            throw new Error(`Agent ${skill.agentId} already has a skill. Each agent can only monetize ONE core skill. Keep everything else free.`);
        }
